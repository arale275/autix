...
class ProfileController {
    ...
    static async getUserProfile(req, res) {
        try {
            ...
            const profile = await profile_model_1.ProfileModel.getUserProfile(userId);
            if (!profile) {
    ...
    static async getFullProfile(req, res) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                return res.status(401).json({
            ...
            const fullProfile = await profile_model_1.ProfileModel.getFullProfile(userId);
            if (!fullProfile) {
                return res.status(404).json({
                    success: false,
                    message: "פרופיל לא נמצא",
    ...
    static async updateUserProfile(req, res) {
        try {
            ...
            const updatedProfile = await profile_model_1.ProfileModel.updateUserProfile(userId, updateData);
            if (!updatedProfile) {
                return res.status(400).json({
                    success: false,
    ...
    static async updateDealerProfile(req, res) {
        try {
            const userId = req.user?.id;
            ...
            const updated = await profile_model_1.ProfileModel.updateDealerProfile(userId, updateData);
            if (!updated) {
                return res.status(400).json({
                    success: false,
            ...
            // החזרת פרופיל מעודכן
            const updatedProfile = await profile_model_1.ProfileModel.getDealerProfile(userId);
            res.json({
                success: true,
    ...
    static async updateBuyerProfile(req, res) {
        try {
            ...
            const updated = await profile_model_1.ProfileModel.updateBuyerProfile(userId, updateData);
            if (!updated) {
                return res.status(400).json({
                    success: false,
            ...
            // החזרת פרופיל מעודכן
            const updatedProfile = await profile_model_1.ProfileModel.getBuyerProfile(userId);
            res.json({
                success: true,
    ...
    static async getProfileStats(req, res) {
        try {
            ...
            const stats = await profile_model_1.ProfileModel.getProfileStats(userId, userType);
            res.json({
                success: true,
                data: stats,
            });
    ...
    static async getAllDealers(req, res) {
        try {
            ...
            let dealers = await profile_model_1.ProfileModel.getAllDealers(limit);
    ...
    static async getDealerById(req, res) {
        try {
            ...
            const dealerProfile = await profile_model_1.ProfileModel.getDealerProfile(userId);
            if (!dealerProfile) {
                return res.status(404).json({
                    success: false,
