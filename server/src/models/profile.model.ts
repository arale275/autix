...
export class ProfileModel {
  ...
  static async getDealerProfile(userId: number): Promise<DealerProfile | null> {
    const query = `
      SELECT 
  ...
  static async getBuyerProfile(userId: number): Promise<BuyerProfile | null> {
    const query = `
      SELECT 
        u.id, u.email, u.first_name, u.last_name, 
        u.phone, u.user_type, u.created_at,
  ...
  static async updateDealerProfile(
    userId: number,
    data: UpdateDealerProfileData
  ): Promise<boolean> {
    const fields = [];
    const values = [];
    let paramCounter = 1;
  ...
  // עדכון העדפות קונה
  static async updateBuyerProfile(
    userId: number,
  ...
  static async getProfileStats(
    ...
    if (userType === "dealer") {
      ...
      const dealerResult = await pool.query(dealerStatsQuery, [userId]);
      ...
      const buyerResult = await pool.query(buyerStatsQuery, [userId]);
    ...
    const activityResult = await pool.query(lastActivityQuery, [userId]);
  ...
  static async getAllDealers(limit: number = 50): Promise<DealerProfile[]> {
    const query = `
      SELECT 
        u.id, u.email, u.first_name, u.last_name, 
        u.phone, u.user_type, u.created_at,
